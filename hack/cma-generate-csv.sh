...
jq_filter="$jq_filter"'.spec.install.spec.deployments[0].spec.template.spec.containers[0].args |= ["-c", "export KEDA_OPERATOR_IMAGE=$RELATED_IMAGE_1; export KEDA_METRICS_SERVER_IMAGE=$RELATED_IMAGE_2; export KEDA_ADMISSION_WEBHOOKS_IMAGE=$RELATED_IMAGE_3; export KEDA_HTTP_ADDON_INTERCEPTOR_IMAGE=$RELATED_IMAGE_4; export KEDA_HTTP_ADDON_OPERATOR_IMAGE=$RELATED_IMAGE_5; export KEDA_HTTP_ADDON_SCALER_IMAGE=$RELATED_IMAGE_6; exec /manager \"$0\" \"$@\"" ] + .  |'
# create a spot to pass in the operand image specs as env vars using OSBS-style RELATED_IMAGE_ names
jq_filter="$jq_filter"'.spec.install.spec.deployments[0].spec.template.spec.containers[0].env += [{"name":"RELATED_IMAGE_1","value":"CMA_OPERAND_PLACEHOLDER_1"},{"name":"RELATED_IMAGE_2","value":"CMA_OPERAND_PLACEHOLDER_2"},{"name":"RELATED_IMAGE_3","value":"CMA_OPERAND_PLACEHOLDER_3"},{"name":"RELATED_IMAGE_4","value":"CMA_OPERAND_PLACEHOLDER_4"},{"name":"RELATED_IMAGE_5","value":"CMA_OPERAND_PLACEHOLDER_5"},{"name":"RELATED_IMAGE_6","value":"CMA_OPERAND_PLACEHOLDER_6"}] | '
# Add taint tolerations to operator deployment to allow it to run on control-plane nodes
jq_filter="$jq_filter"'.spec.install.spec.deployments[0].spec.template.spec.containers[0].tolerations += [{"effect":"NoSchedule","key":"node-role.kubernetes.io/master","operator":"Exists"},{"effect":"NoSchedule","key":"node-role.kubernetes.io/control-plane","operator":"Exists"}] | '
